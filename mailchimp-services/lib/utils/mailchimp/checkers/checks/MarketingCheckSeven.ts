...
export class MarketingCheckSeven extends CreditReportChecker {
  ...
  check(): IMarketingCheckerResults {
    ...
    if (current.metrics.countAllAccounts > prior.metrics.countAllAccounts) {
      const tags = [this.generateTag('added_account(s)', 'active'), this.generateTag('removed_account(s)', 'inactive')];
      return this.generateResults(true, tags);
    } else if (current.metrics.countAllAccounts < prior.metrics.countAllAccounts) {
      const tags = [this.generateTag('added_account(s)', 'inactive'), this.generateTag('removed_account(s)', 'active')];
      return this.generateResults(true, tags);
    } else {
      const tags = [
        this.generateTag('added_account(s)', 'inactive'),
        this.generateTag('removed_account(s)', 'inactive'),
      ];
      return this.generateResults(true, tags);
    }
  }
