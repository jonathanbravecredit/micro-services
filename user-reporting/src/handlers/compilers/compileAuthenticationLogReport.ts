...
export const main: Handler<any, any> = async (event: any): Promise<any> => {
  try {
    ...
    const reportId = ReportNames.AuthenticationAll;
    ...
    const auths = opsreports.map((report, i) => {
      if (i < 2) {
    ...
    const csvAllData = csvjson.toCSV(JSON.stringify(auths), {
      headers: 'key',
    });
    const emails = STAGE === 'dev' ? ['jonathan@brave.credit'] : ['jonathan@brave.credit'];
    let params = generateEmailParams(`authenticationLogReport`, emails);
    params.attachments = [
      {
        filename: 'prodDBReportAuthenticationLog.csv',
        content: csvAllData,
      },
    ];
