...
export const main: Handler<any, any> = async (event: any): Promise<any> => {
  try {
    ...
    const reportId = ReportNames.MissingDisputeKeys;
    ...
    const missingKeys = opsreports.map((report) => {
      const data = JSON.parse(report.record);
      return data;
    });
    // send an email letting me know which segment is done

    const csvAllData = csvjson.toCSV(JSON.stringify(missingKeys), {
      headers: 'key',
    });
    const emails = STAGE === 'dev' ? ['jonathan@brave.credit'] : ['jonathan@brave.credit'];
    let params = generateEmailParams(`MissingDisputeKeys`, emails);
    params.attachments = [
      {
        filename: `prodDBMissingKeysReport-${batchId}.csv`,
        content: csvAllData,
      },
    ];
