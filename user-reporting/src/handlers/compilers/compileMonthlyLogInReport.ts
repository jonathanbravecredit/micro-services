...
export const main: Handler<any, any> = async (event: any): Promise<any> => {
  try {
    ...
    const reportId = ReportNames.MonthlyLogIn;
    ...
    const mothlyLogIns = opsreports.map((report) => {
      const data = JSON.parse(report.record);
      return data;
    });
    // send an email letting me know which segment is done

    const csvAllData = csvjson.toCSV(JSON.stringify(mothlyLogIns), {
      headers: 'key',
    });
    const emails = STAGE === 'dev' ? ['jonathan@brave.credit', 'noah@brave.credit'] : ['jonathan@brave.credit'];
    let params = generateEmailParams(`monthlyLogInReport`, emails);
    params.attachments = [
      {
        filename: 'prodDBReportMonthlyLogIn.csv',
        content: csvAllData,
      },
    ];
