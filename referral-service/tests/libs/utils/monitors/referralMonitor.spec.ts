...
describe('ReferralMonitor', () => {
  ...
  describe('monitor', () => {
    it('should call monitorDynamo and monitorSns', async () => {
      const spyDb = jest.spyOn(monitor, 'monitorDynamo').mockReturnValueOnce(Promise.resolve());
      const spySns = jest.spyOn(monitor, 'monitorSns').mockReturnValueOnce(Promise.resolve());
  ...
  describe('monitorSns', () => {
    it('should NOT call checkActivations if no records', async () => {
      const spy = jest.spyOn(monitor, 'checkActivations').mockReturnValueOnce(Promise.resolve());
    ...
    it('should call checkActivations', async () => {
      const mock = {
        EventSource: 'aws:sns',
        Sns: { Subject: 'sessiondataupdate', Message: '{ "service": "referralservice" }' },
  ...
  describe('checkActivations', () => {
    ...
    it('should call the init and check methods on ReferralActivationManager instance if subject = "transunionenrollment"', async () => {
      await monitor.checkActivations({ Sns: { Subject: 'transunionenrollment' } } as any);
      const mock1 = mockedActMgr.mock.instances[0].init;
      const mock2 = mockedActMgr.mock.instances[0].check;
      expect(mock1).toHaveBeenCalled();
      expect(mock2).toHaveBeenCalled();
