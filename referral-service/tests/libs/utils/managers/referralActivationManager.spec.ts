...
describe('ReferralActivationManager', () => {
  const manager = new ReferralActivationManager({ Sns: { Message: null } } as any, '' as any);
  const h = new Helper<ReferralActivationManager>(manager);
  const mockedGetCamp = mocked(getCampaign);
  const mockedGetRef = mocked(getReferral);
  const mockedCreateRef = mocked(createReferral);
  const mockedUpdateEnroll = mocked(updateEnrollment);
  const mockedUpdateRefCamp = mocked(updateReferralCampaign);
  const mockedUpdateRefElig = mocked(updateReferralEligibility);
  const mockedListSessions = mocked(listUserSessions);
  ...
  describe('init', () => {
    it('should call initSessionData if subject = "sessiondataupdate"', async () => {
      const mgr = new ReferralActivationManager({ Sns: { Message: null } } as any, 'sessiondataupdate');
    ...
    it('should call initApplicationData if subject = "transunionenrollment"', async () => {
      const mgr = new ReferralActivationManager({ Sns: { Message: null } } as any, 'transunionenrollment');
      const spy = jest.spyOn(mgr, 'initApplicationData');
      await mgr.init();
      expect(spy).toHaveBeenCalled();
    });
  ...
  describe('initSessionData', () => {
    it('should NOT call getReferral, listSessions, getCampaign if Message, message, or userId is falsey', async () => {
      ...
      const mgr1 = new ReferralActivationManager({ Sns: { Message: null } } as any, 'sessiondataupdate');
    ...
    it('should call getReferral, listSessions, getCampaign if Message, message, and userId are truthy', async () => {
      const mock = { Sns: { Message: '{ "message": { "userId": "abc" } }' } } as any;
  ...
  describe('initApplicationData', () => {
    it('should NOT call getReferrl if Message, message, or id is falsey', async () => {
      const spy = jest.spyOn(console, 'error');
      const mgr1 = new ReferralActivationManager({ Sns: { Message: null } } as any, 'transunionenrollment');
      const mgr2 = new ReferralActivationManager(
        { Sns: { Message: '{ "message": null }' } } as any,
    ...
    it('should call getReferral if Message, message, and userId are truthy', async () => {
      const mock = { Sns: { Message: '{ "message": { "id": "abc" } }' } } as any;
  ...
  describe('check method', () => {
    describe('sessiondataupdate path', () => {
      ...
      let mgr: any;
      beforeEach(() => {
        mock = { Sns: { Message: '{ "message": { "userId": "abc" } }' } } as any;
        mgr = new ReferralActivationManager(mock, 'sessiondataupdate');
      });
      it('should call checkSessionData if id set', async () => {
        const spy = jest.spyOn(mgr, 'checkSessionData').mockReturnValueOnce(false);
        await mgr.init();
        await mgr.check();
        expect(spy).toHaveBeenCalled();
        spy.mockReset();
  ...
  describe('check method', () => {
    describe('transunionenrollment path', () => {
      ...
      it('should call checkApplicationData if id set', async () => {
        const spy = jest.spyOn(mgr, 'checkApplicationData').mockReturnValueOnce(false);
        await mgr.init();
        await mgr.check();
        expect(spy).toHaveBeenCalled();
        spy.mockReset();
