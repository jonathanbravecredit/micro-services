...
export const main: APIGatewayProxyHandler = async (
  ...
  const validate = ajv.getSchema<IGetReferralByCampaign>(
    "referralByCampaignGetAll"
  );
  ...
  try {
    ...
    if (!referralCode || referral.status === "suspended")
      return response(200, null);
    const allReferrals = await getAllReferralsByCampaign(
      referralCode,
      campaign
    );
