...
import {
  getReferral,
  getAllEnrolledReferralsByMonth,
  createReferral,
} from "lib/queries";
import {
...
import {
  createBlankMonthlyReferral,
  groupReferralsByYearMonth,
} from "lib/utils/referrals/referral.utils";
...
export const main: APIGatewayProxyHandler = async (
  ...
  const validate =
    ajv.getSchema<interfaces.IGetReferralEarningsByCampaignMonthly>(
  ...
  try {
    ...
    const allReferrals = await getAllEnrolledReferralsByMonth(
      code,
      campaign,
      month,
      year
