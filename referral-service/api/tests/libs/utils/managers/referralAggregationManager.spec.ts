...
import { ReferralQueries, Campaign, Referral } from "@bravecredit/brave-sdk";
...
describe("ReferralAggregationManager Class", () => {
  ...
  const mockedUpdate = mocked(ReferralQueries.updateReferral);
  const mockedGetCode = mocked(ReferralQueries.getReferralByCode);
  ...
  describe("getReferrer", () => {
    it("should return null if currImage.referredByCode is null", async () => {
      const agg = new ReferralAggregationManager(
        MOCK_CAMPAIGN_ACTIVE,
        MOCK_NOTREFERRED_MODIFY
      );
      await agg.init();
  ...
  describe("setEnrollment", () => {
    ...
    it('should set enrollment to "not_enrolled" when NOT curr enrolled and priorImage is undefined', async () => {
      let agg = new ReferralAggregationManager(
        mockCampaign,
        MOCK_UNENROLLED_INSERT
      );
      await agg.init();
  ...
  describe("quantifyReferral", () => {
    describe("new enrollments", () => {
      let aggTwo = new ReferralAggregationManager(
        MOCK_CAMPAIGN_ACTIVE,
        MOCK_UNENROLLED_TO_ENROLLED_MODIFY
      );
      ...
      it("should not call creditReferee or creditReferrer if campaign is inactive", async () => {
        aggTwo = new ReferralAggregationManager(
          MOCK_CAMPAIGN_NO_CAMPAIGN,
          MOCK_UNENROLLED_TO_ENROLLED_MODIFY
        );
    ...
    it("should NOT call creditReferrer nor creditReferree if campaign = NO_CAMPAIGN", async () => {
      let aggThree = new ReferralAggregationManager(
        MOCK_CAMPAIGN_NO_CAMPAIGN,
        MOCK_MODIFY
      );
  ...
  describe("creditReferrer", () => {
    ...
    let updateRefSpy = jest
      .spyOn(aggregator, "updateReferral")
  ...
  describe("getPaymentDate", () => {
    ...
    it("should call PaymentDateCalculate.calcPaymentDate", () => {
      ...
      aggregator.getPaymentDate({} as Referral);
      expect(mockedPayCalc.calcPaymentDate).toHaveBeenCalledWith(
        false,
        endDate
      );
    });
  ...
  describe("hitMax", () => {
    it("should return false if referred falsey", () => {
      const res1 = aggregator.hitMax({
        campaignActiveReferred: 0,
      } as unknown as Referral);
      const res2 = aggregator.hitMax({
        campaignActiveReferred: -1,
      } as unknown as Referral);
      const res3 = aggregator.hitMax({
        campaignActiveReferred: false,
      } as unknown as Referral);
      const res4 = aggregator.hitMax({
        campaignActiveReferred: undefined,
      } as unknown as Referral);
