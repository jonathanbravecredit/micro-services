"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.DataBreachConditions = void 0;
const constants_1 = require("../constants");
const filters_1 = require("../filters");
const dataBreachCondition1 = ({ address, tradelines, }) => {
    var _a, _b;
    if (((_b = (_a = address === null || address === void 0 ? void 0 : address.CreditAddress) === null || _a === void 0 ? void 0 : _a.stateCode) === null || _b === void 0 ? void 0 : _b.toLowerCase()) !== 'ca')
        return constants_1.DataBreaches.None;
    const carLoan = filters_1.TransunionFilters.filterTradelinesByIndustryCode(tradelines, 'BA');
    if (carLoan.length > 0)
        return constants_1.DataBreaches.Condition1;
    return constants_1.DataBreaches.None;
};
const dataBreachCondition2 = ({ address }) => {
    var _a, _b;
    if (((_b = (_a = address === null || address === void 0 ? void 0 : address.CreditAddress) === null || _a === void 0 ? void 0 : _a.stateCode) === null || _b === void 0 ? void 0 : _b.toLowerCase()) === 'ca')
        return constants_1.DataBreaches.Condition2;
    return constants_1.DataBreaches.None;
};
const dataBreachCondition3 = ({ address }) => {
    var _a, _b;
    if (((_b = (_a = address === null || address === void 0 ? void 0 : address.CreditAddress) === null || _a === void 0 ? void 0 : _a.stateCode) === null || _b === void 0 ? void 0 : _b.toLowerCase()) === 'co')
        return constants_1.DataBreaches.Condition3;
    return constants_1.DataBreaches.None;
};
const dataBreachCondition4 = ({ address }) => {
    var _a, _b;
    const code = (_b = (_a = address === null || address === void 0 ? void 0 : address.CreditAddress) === null || _a === void 0 ? void 0 : _a.stateCode) === null || _b === void 0 ? void 0 : _b.toUpperCase();
    if (!code)
        return constants_1.DataBreaches.None;
    return krogerStates[code] ? constants_1.DataBreaches.Condition4 : constants_1.DataBreaches.None;
};
const dataBreachCondition5 = ({ inquiries }) => {
    const results = inquiries.filter((i) => {
        var _a;
        const name = ((_a = i === null || i === void 0 ? void 0 : i.Inquiry) === null || _a === void 0 ? void 0 : _a.subscriberName) || '';
        if (name.toLowerCase().indexOf('t-mobile') >= 0)
            return true;
        if (name.toLowerCase().indexOf('tmobile') >= 0)
            return true;
        if (name.toLowerCase().indexOf('t mobile') >= 0)
            return true;
        return false;
    });
    if (results.length > 0)
        return constants_1.DataBreaches.Condition5;
    return constants_1.DataBreaches.None;
};
const dataBreachCondition6 = ({ address }) => {
    var _a, _b;
    if (((_b = (_a = address === null || address === void 0 ? void 0 : address.CreditAddress) === null || _a === void 0 ? void 0 : _a.stateCode) === null || _b === void 0 ? void 0 : _b.toLowerCase()) === 'wa')
        return constants_1.DataBreaches.Condition6;
    return constants_1.DataBreaches.None;
};
const dataBreachCondition7 = ({ address }) => {
    var _a, _b;
    const code = (_b = (_a = address === null || address === void 0 ? void 0 : address.CreditAddress) === null || _a === void 0 ? void 0 : _a.stateCode) === null || _b === void 0 ? void 0 : _b.toUpperCase();
    if (!code)
        return constants_1.DataBreaches.None;
    return experianStates[code] ? constants_1.DataBreaches.Condition7 : constants_1.DataBreaches.None;
};
exports.DataBreachConditions = {
    [constants_1.DataBreaches.Condition1]: dataBreachCondition1,
    [constants_1.DataBreaches.Condition2]: dataBreachCondition2,
    [constants_1.DataBreaches.Condition3]: dataBreachCondition3,
    [constants_1.DataBreaches.Condition4]: dataBreachCondition4,
    [constants_1.DataBreaches.Condition5]: dataBreachCondition5,
    [constants_1.DataBreaches.Condition6]: dataBreachCondition6,
    [constants_1.DataBreaches.Condition7]: dataBreachCondition7,
};
const krogerStates = {
    AL: true,
    AR: true,
    AZ: true,
    CA: true,
    CO: true,
    GA: true,
    NC: true,
    IA: true,
    ID: true,
    IL: true,
    IN: true,
    KS: true,
    KY: true,
    LA: true,
    MD: true,
    MI: true,
    MO: true,
    MT: true,
    NE: true,
    NM: true,
    NV: true,
    OH: true,
    OR: true,
    PA: true,
    SC: true,
    TN: true,
    TX: true,
    UT: true,
    VA: true,
    WA: true,
    WI: true,
    WV: true,
    WY: true,
};
const experianStates = {
    ND: true,
    SD: true,
    WI: true,
    OK: true,
    ME: true,
    VT: true,
    NH: true,
    NY: true,
    MA: true,
    NJ: true,
    DE: true,
    CT: true,
    RI: true,
    HI: true,
    AK: true,
    NC: true,
    FL: true,
};
