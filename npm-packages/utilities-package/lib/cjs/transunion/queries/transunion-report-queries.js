...
const common_tu_interface_1 = require("../interfaces/common-tu.interface");
...
class TransunionReportQueries extends transunion_missing_1.TransunionMissing {
    ...
    static getBraveTradelineDescription(partition) {
        ...
        const status = constants_1.BRAVE_ACCOUNT_TYPE[`${(_b = (_a = partition.Tradeline) === null || _a === void 0 ? void 0 : _a.PayStatus) === null || _b === void 0 ? void 0 : _b.symbol}`];
    ...
    static getTradelineTypeDescription(partition) {
        var _a;
        if (!partition)
            return common_tu_interface_1.AccountTypes.Unknown;
        const description = common_tu_interface_1.ACCOUNT_TYPES[`${(_a = partition.accountTypeSymbol) === null || _a === void 0 ? void 0 : _a.toLowerCase()}`];
        return description ? description : common_tu_interface_1.AccountTypes.Unknown;
    }
    /**
    ...
    static isForbearanceAccount(partition) {
        ...
        if (!accountTypeSymbol)
            return false;
        const accountType = constants_1.FORBEARANCE_TYPE[accountTypeSymbol.toLowerCase()];
        if (!accountType)
            return false;
    ...
    static isNegativeAccount(partition) {
        ...
        if (!symbol)
            return false;
        return !!constants_1.NEGATIVE_PAY_STATUS_CODES[`${symbol}`] || false;
    }
    /**
     * Helper function to securely lookup the account type
     * @param {ITradeLinePartition | undefined} partition
    ...
    static isPositiveAccount(partition) {
        ...
        if (!symbol)
            return false;
        return !!constants_1.POSITIVE_PAY_STATUS_CODES[`${symbol}`] || false;
    }
    /**
    ...
    static isDataBreachCondition(report, condition) {
        var _a;
        if (!report)
            return constants_1.DataBreaches.None;
        ...
        switch (condition) {
            case constants_1.DataBreaches.Condition1:
                // Check if living in California and have car loans
                return conditions_1.DataBreachConditions[constants_1.DataBreaches.Condition1]({ address, tradelines });
                break;
            case constants_1.DataBreaches.Condition2:
                // Check if living in California
                return conditions_1.DataBreachConditions[constants_1.DataBreaches.Condition2]({ address });
                break;
            case constants_1.DataBreaches.Condition3:
                // Check if living in Colorado
                return conditions_1.DataBreachConditions[constants_1.DataBreaches.Condition3]({ address });
                break;
            case constants_1.DataBreaches.Condition4:
                // Check if living in a Kroger state
                return conditions_1.DataBreachConditions[constants_1.DataBreaches.Condition4]({ address, tradelines });
                break;
            case constants_1.DataBreaches.Condition5:
                // Check for soft/hard inquiries with T Mobile
                return conditions_1.DataBreachConditions[constants_1.DataBreaches.Condition5]({ inquiries });
                break;
            case constants_1.DataBreaches.Condition6:
                // Check if living in Washington
                return conditions_1.DataBreachConditions[constants_1.DataBreaches.Condition6]({ address });
                break;
            case constants_1.DataBreaches.Condition7:
                // Check if living in Washington
                return conditions_1.DataBreachConditions[constants_1.DataBreaches.Condition7]({ address });
                break;
            default:
                return constants_1.DataBreaches.None;
                break;
    ...
    static listDataBreaches(report) {
        const breachCards = Object.values(constants_1.DataBreaches)
            .filter((item) => {
            return this.isDataBreachCondition(report, item) !== constants_1.DataBreaches.None;
        })
            .map((key) => {
            return constants_1.DateBreachCard[key];
        });
        return breachCards;
    }
    /*=====================================*/
