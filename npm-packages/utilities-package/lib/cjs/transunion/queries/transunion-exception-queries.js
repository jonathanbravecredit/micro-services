...
class TransunionExceptionQueries {
    ...
    static getErrorCodeDetails(code) {
        const detail = constants_1.TRANSUNION_ERROR_CODES[code];
        return !detail
            ? constants_1.TRANSUNION_ERROR_CODES['11'] // non-specific error
            : detail;
    }
    static isErrorCritical(resp) {
        const { error: { Code, Message, Name } = {} } = resp;
        const { keyWords } = constants_1.TRANSUNION_CRITICAL_ERRORS[`${Code}`] || {};
        if (!keyWords)
            return false;
