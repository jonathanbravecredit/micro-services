...
class CreditMixMetric {
    ...
    getRatingsOnly() {
        const recs = Object.values(transunion_1.CreditMixRecommendations);
        for (let i = 0; i < recs.length; i++) {
            if (transunion_1.RecommendationConditionalLogic[recs[i]](this.summary)) {
                const { rating, color } = transunion_1.RecommendationValues[recs[i]];
                return {
                    rating,
                    color,
                };
    ...
    getRecommendations() {
        ...
        for (let i = 0; i < recs.length; i++) {
            if (transunion_1.RecommendationConditionalLogic[recs[i]](this.summary)) {
                return transunion_1.RecommendationValues[recs[i]];
            }
        }
    }
