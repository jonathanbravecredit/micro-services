// import { AccountTypes, CreditMixRecommendations, DataBreaches, IBreachCard, CreditReportGroups, CreditMixRecommendationsCodes } from '../types';
...
export enum CreditMixRecommendationsCodes {
...
export const DATA_BREACH_CARDS: Record<any, IBreachCard> = {
...
export const CREDIT_MIX_CONDITIONS: Record<string, (arg: ICreditMixTLSummary) => boolean> = {
  condition1: (summary: ICreditMixTLSummary) => {
    const { totalLineAmount, amountOfClosed } = summary;
    return !amountOfClosed && !totalLineAmount;
  },
  condition2: (summary: ICreditMixTLSummary) => {
    const { totalLineAmount } = summary;
    return totalLineAmount === 1;
  },
  condition3: (summary: ICreditMixTLSummary) => {
    const { totalLineAmount, amountOfClosed } = summary;
    return totalLineAmount - amountOfClosed <= 4 && totalLineAmount - amountOfClosed >= 2;
  },
  condition4: (summary: ICreditMixTLSummary) => {
  ...
  condition5: (summary: ICreditMixTLSummary) => {
    const { totalLineAmount, amountOfClosed } = summary;
    return totalLineAmount === amountOfClosed && amountOfClosed > 2;
  },
  condition6: (summary: ICreditMixTLSummary) => {
  ...
  condition7: (summary: ICreditMixTLSummary) => {
    const { autoLoanAmount, studentLoanAmount, mortgageAmount, totalLineAmount, amountOfClosed } = summary;
    return totalLineAmount - amountOfClosed <= 7 && (!!studentLoanAmount || !!autoLoanAmount || !!mortgageAmount);
  },
  condition8: (summary: ICreditMixTLSummary): boolean => {
  ...
  condition9: (summary: ICreditMixTLSummary) => {
    const { totalLineAmount, amountOfClosed } = summary;
    return totalLineAmount - amountOfClosed >= 8;
  },
};

export const CREDIT_MIX_RECOMMENDATION_LOGIC: Record<string, (summary: ICreditMixTLSummary) => boolean> = {
  [CreditMixRecommendationsCodes.NoClosedAndNoOpen]: CREDIT_MIX_CONDITIONS.condition1,
  [CreditMixRecommendationsCodes.OnlyOneOpen]: CREDIT_MIX_CONDITIONS.condition2,
  [CreditMixRecommendationsCodes.TwoToFourOpen]: CREDIT_MIX_CONDITIONS.condition3,
  [CreditMixRecommendationsCodes.FivePlusOnlyCC]: CREDIT_MIX_CONDITIONS.condition4,
  [CreditMixRecommendationsCodes.AllClosed]: CREDIT_MIX_CONDITIONS.condition5,
  [CreditMixRecommendationsCodes.SevenOrLessNoMortgage]: CREDIT_MIX_CONDITIONS.condition6,
  [CreditMixRecommendationsCodes.SevenOrLess]: CREDIT_MIX_CONDITIONS.condition7,
  [CreditMixRecommendationsCodes.EightOrMoreAtLeastOneOfAll]: CREDIT_MIX_CONDITIONS.condition8,
  [CreditMixRecommendationsCodes.EightOrMore]: CREDIT_MIX_CONDITIONS.condition9,
};

export const CREDIT_MIX_RECOMMENDATON_VALUES: Record<string, IRecommendationText> = {
...
export const STUDENT_LOAN_INDUSTRY_CODES: Record<string, string> = {
  bs: 'student_loan',
  el: 'student_loan',
  vg: 'student_loan',
};
export const KROGER_STATES: Record<any, boolean> = {
  AL: true,
  AR: true,
  AZ: true,
  CA: true,
...
export const EXPERIAN_STATES: Record<any, boolean> = {
  ND: true,
  SD: true,
  WI: true,
  OK: true,
