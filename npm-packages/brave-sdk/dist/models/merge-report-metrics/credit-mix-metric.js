...
class CreditMixMetric {
    ...
    getMetric() {
        ...
        return new credit_report_metrics_1.CreditReportMetric("credit_mix" /* CreditMix */, "Credit Mix" /* CreditMix */, rating, status);
    }
    get summary() {
    ...
    getRatingsOnly() {
        const recs = Object.values(transunion_1.CreditMixRecommendationsCodes);
        for (let i = 0; i < recs.length; i++) {
            if (transunion_1.CREDIT_MIX_RECOMMENDATION_LOGIC[recs[i]](this.summary)) {
                const { rating, color } = transunion_1.CREDIT_MIX_RECOMMENDATON_VALUES[recs[i]];
                return {
                    rating,
                    color,
                };
    ...
    getRecommendations() {
        ...
        for (let i = 0; i < recs.length; i++) {
            if (transunion_1.CREDIT_MIX_RECOMMENDATION_LOGIC[recs[i]](this.summary)) {
                return transunion_1.CREDIT_MIX_RECOMMENDATON_VALUES[recs[i]];
            }
        }
    }
...
class CreditMixMetricSummaryMaker {
    constructor() {
...
exports.CreditMixMetricSummaryMaker = CreditMixMetricSummaryMaker;
